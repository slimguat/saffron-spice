...
class LockProtocols():
    ...
    def add_lock(self,target_line_ind, lock_line_ind, wvl ):
        if not isinstance(lock_line_ind,Iterable): lock_line_ind = [lock_line_ind] 
        if not isinstance(wvl          ,Iterable): wvl           = [wvl          ] 
        ...
        if self.lock:
            ...
            for target,lock_option in self.lock.items():
                if target_line_ind in lock_option: raise Exception (f"{target_line_ind} is already a lock option groupe them under same target please")
                for existing_lock_line in [tab[0] for tab in lock_option]:
                    if existing_lock_line in lock_line_ind :
                        raise Exception(f'The value  Does exist already delete it first in one lock options')
    ...
    def add_block(self,target_line_ind, wvl ):
        if not isinstance(target_line_ind,Iterable): target_line_ind=[target_line_ind]
        if not isinstance(wvl            ,Iterable): wvl            =[wvl            ]
...
def gen_locked_params(unlocked_init_params,unlocked_quentities,lock_protocols,unlocked_cov=None):
    assert not (
        len(unlocked_init_params)==0 or 
        len(unlocked_quentities )==0  or
        len(unlocked_init_params)!=len(unlocked_quentities )==0  
        )
    ...
    if not unlocked_cov is None:unlocked_cov         = list(unlocked_cov)
    locked_init_params = unlocked_init_params.copy()
    if not unlocked_cov is None:locked_cov         = unlocked_cov        .copy()
    ...
    for target, lines in lock_protocols.lock.items():
        for line in lines:
            ...
            if not unlocked_cov is None: locked_cov        .pop(ind_line)
            ...
            ind_line4unlock_code = find_nth_occurrence(unlocked_quentities,"I",line[0]+1)+1
            ind_target =  find_nth_occurrence(unlocked_quentities,"I",target+1)+1
            ind_comma = find_nth_occurrence(code_lock2unlock,',',ind_line4unlock_code+1)
            ind_target2 = ind_target - len([lin for lin in lines_done if line>lin ]) + unlocked_quentities[:ind_target].count("B")
            code_lock2unlock = (
                code_lock2unlock[:ind_comma]+
                "{0}"+"["+str(ind_target2)+']'+'+'+str(line[1])+
                code_lock2unlock[ind_comma:])
    ...
    while iter<code_lock2unlock.count(","):
        ...
        if code_lock2unlock[ind_beg1+1:ind_end1]=="":
            code_lock2unlock = code_lock2unlock[:ind_beg1+1] +"{0}"+"["+str(ind_array)+"]"+code_lock2unlock[ind_end1:] 
            code_unlock2lock = code_unlock2lock[:ind_beg2+1] +"{0}"+"["+str(iter-1   )+"]"+code_unlock2lock[ind_end2:] 
            ind_array+=1
        iter+=1
    
    code_lock2unlock='['+code_lock2unlock[1:]
    code_unlock2lock='['+code_unlock2lock[1:]
    if unlocked_cov is None:return (np.array(locked_init_params),locked_quentities           ,code_lock2unlock,code_unlock2lock)
    else                   :return (np.array(locked_init_params),locked_quentities,np.array(locked_cov),code_lock2unlock,code_unlock2lock)
def gen_unlocked_params(locked_init_params,locked_quentities,lock_protocols,locked_cov=None):
    assert not (
        len(locked_init_params)==0 or 
        len(locked_quentities )==0  or
        len(locked_init_params)!=len(locked_quentities )==0  
        )
    if not locked_cov is None: locked_cov = list(locked_cov) 
    ...
    if not locked_cov is None: unlocked_cov = locked_cov.copy() 
    ...
    for target, lines in lock_protocols.lock.items():
        for line in lines:
            ...
            unlocked_init_params.insert(ind_line,unlocked_init_params[ind_target]+line[1])
            if not locked_cov is None:unlocked_cov.insert(ind_line,unlocked_cov[ind_target])
            
    if locked_cov is None:return (np.array(unlocked_init_params),unlocked_quentities             )
    else:                 return (np.array(unlocked_init_params),unlocked_quentities,unlocked_cov)
def gen_lock_fit_func(unlocked_init_params,unlocked_quentities,lock_protocols,inner_fit_func):
    _,_,code_lock2unlock,code_unlock2lock = gen_locked_params(unlocked_init_params,unlocked_quentities,lock_protocols)
    ...
    \n    return y
    """.format(time_str,code_lock2unlock.format('array'),inner_fit_func.__name__,code_unlock2lock.format('y'))
    # print(str_func)
