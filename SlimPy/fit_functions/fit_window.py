...
from ..utils import fst_neigbors,join_dt,Preclean,gen_shmm,verbose_description,convolve
from ..utils.denoise import denoise_data, denoise_raster
...
def _fit_window(x:np.ndarray,
    ...
    """Fitting a whole window of dimension (n_t x n_lmb x n_y x n_x) and        creating a map of fitting parameters
        ...
        TODO 
            inject noise and weights to the algorithm 
        """
    if lock == None:
        lock = Lock()
    # lock.acquire()
    ...
    if True:
        ...
        if type(wgt) != type(None):
            shmm_wgt,data_wgt = gen_shmm(create=False,
                                    name  = wgt["name"], 
                                    dtype = wgt["type"], 
                                    shape = wgt["shape"] 
                                    )
    ...
    if True:# despiking the results
        window = despike(raw_data=window,
    ...
    if True: #creating pixel convolution matrices (always adaptive) 
        ...
        conv_data = convolve(window=window,
                 mode=mode,
        ...
        # #Generating denoised maps
        # if type(denoise) != type(None):
        #     if verbose>=1: print(f'Generating denoised maps with denoise sigma => {denoise}')
        #     for i in range(convolution_extent_list.shape[0]):
        #         Dnois_conv_data = denoise_data(conv_data[i,0],denoise_sigma=denoise)
        #         conv_data[i,0] = Dnois_conv_data.copy()
...
def task_fit_pixel(x:np.ndarray,
    ...
    shmm_con,data_con = gen_shmm(create=False,name=con["name"],dtype=con["type"],shape=con["shape"]) 
    if type(wgt)!=type(None):
        shmm_wgt,data_wgt = gen_shmm(create=False,name=wgt["name"],dtype=wgt["type"],shape=wgt["shape"]) 
        ...
        while True: #this will break only when the convolution threshold is met or reached max allowed convolutions
            ...
            if data_war[i_ad,0,:,i_y,i_x][np.logical_not(np.isnan(data_war[i_ad,0,:,i_y,i_x]))].shape[0]>=ini_params.shape[0]:
                last_par,last_cov = fit_pixel_multi(x =x,
                                                ...
                                                fit_func=fit_func,
                                                bounds=bounds,
                                                weights=None if type(wgt)==type(None) else data_wgt[0,:,i_y,i_x],
                                                verbose=verbose,
