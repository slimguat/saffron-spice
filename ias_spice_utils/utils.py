...
def read_uio_cat(data_path):
    ...
    * `df[(df.LEVEL == "L2") & (df.STUDYDES == "Standard dark for cruise phase")]`
    ...
    df = pd.read_table(cat_file, skiprows=1, names=columns, na_values="MISSING",
                       parse_dates=date_columns, on_bad_lines='warn',low_memory=False )
...
def write_studies_files_list_for_stp(stp_number, data_path, out_path):
    ...
    stp_number_int=int(stp_number)
    print ( df.LEVEL, '  ', df["STP"], ' type ', type(df["STP"]) )

    l2 = df[(df.LEVEL == "L2") &
            (df["STP"] == stp_number_int)]
    ...
    print('writing files_list.txt in ', out_path);
...
def make_notebooks_for_stp(stp_number, complete):
    """
    ...
    if complete:
        ...
        xx = stp_name+"_incomplete"
    stp_dir = Path(notebooks_root) / xx

    print('stp_dir = ', stp_dir)

    rtc = write_studies_files_list_for_stp(stp_number, data_path, stp_dir)
    if rtc:
        print('no study found for this STP ', stp_number)
        return 1 
    studies_file = Path(notebooks_root) / stp_dir / 'files_list.txt'
    for study in read_studies_files_list_for_stp(studies_file).keys():
        make_notebook_from_template(study, stp_number, stp_dir)
    return 0

def process_notebooks_for_stp(stp_number, complete):
    if complete:
        stp_dir = 'STP' + stp_number
    else:
        stp_dir = 'STP' + stp_number +	'_incomplete'
    notebooks_root = os.getenv("NOTEBOOKS_ROOT",default='/home/nb/pre-analysis')
    path = Path(notebooks_root) / stp_dir
...
def unique_windows(raster):
    ...
    """
    headers = [r.header for r in raster]
...
def make_summary(raster, filename, n_columns=3, figsize=(12, 8)):
    ...
    for iw, kw in enumerate(unq):
        ...
        if np.all(np.isnan(window.data)):
            raise ValueError(f'empty hdu: {kw}')

        if window.data.shape[3] == 1:
            data = np.nanmean(window.data, axis=(0, 3)).T
        else:
            if 'w-ras' in filename:  # wide raster -> put windows side by side
                half_win_wid = (window.header['CDELT1'] /
                                window.header['CDELT2']) / 2
                half_win_wid = int(np.round(half_win_wid))
                win_cen = window.data.shape[1] // 2
                data = window.data[:, win_cen-half_win_wid:win_cen+half_win_wid+1]
                data = np.moveaxis(data, 1, 3).reshape(
                    window.data.shape[2], -1)
