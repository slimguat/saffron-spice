...
def fit_pixel_multi(x:np.ndarray,
    ...
    if np.any(np.isnan(w)) : print(f"x     : {_x}\ny     : {_y}\nsigma : {w}")
    
    
    if _y.shape[0]<=_s:
        if verbose>=1: print("after cleaning data the number of parameters is greater than data points\nPrecleaning size  = {}\nPostcleaning size = {}nans size         = {}".format(
            y.shape[0],
            _y.shape[0],
    ...
    try:
        ...
        if verbose>=1: print("couldn't find the minimum"+("Plotting the result in ./tmp" if plotit else ""))
    ...
    if plotit or verbose>=3 or verbose==-3:
        ...
        axis.plot(
            _x,
            fit_func(_x,*ini_params),":",label="initial params")
        ...
        if kwargs["lock_protocols"]:
            last_par,locked_quentities = gen_unlocked_params(res[0],quentities,kwargs['lock_protocols'])
            locked_quentities = np.array(locked_quentities)
            lbd = last_par[locked_quentities=="x"]
            for i,l in enumerate(lbd):
                plt.axvline(l,label=f"{l:05.1}",ls=":",color=np.random.random(size=(3)))
        axis.set_xlabel(xlabel)        
        axis.legend(fontsize = 12)
        
        plt.tight_layout()
        
        plt.savefig("./tmp/{}.jpg".format( 
                kwargs['plot_title_prefix'],
