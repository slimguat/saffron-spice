    ...
    def check_object_arguments(self):
        ...
        for i in range(len(self.init_params)):
            if len(self.init_params[i])!=len(self.quentities[i]):
                print(f'initial parameters provided are not aligned with the quentities\nindex: {i}\ninit parms: {self.init_params[i]}\nquentities: {self.quentities}')
...
class ProgressFollower():
    def __init__(self,file_path=None):
        ...
        if file_path is None:
            ...
            filename = "Progress_Shmm_config_"+str(datetime.datetime.now()).replace('-','').replace('.','').replace(' ','T').replace(':','')+".p"
    ...
    def launch(self):
        with self.pickle_lock:
            log= pickle.load(open(self.file_path,"rb"))
        shmm,data = gen_shmm(create=False,**log["is_launched"])
        if data[0]==1: print('Progress is already tracked another one will be launched')
        data[0] = 1
        print('launching following progress')
        print('from file ', self.file_path)
        self.process = Process(target = self.follow_func, args=(self.file_path,self.pickle_lock))
        self.process.start()
    def __enter__(self):
    ...
    def __del__(self):
        ...
        if self.process.is_alive():
            print("Progress follower process is still running. Proceeding terminating it.")
        ...
        with Progress() as progress:
            ...
            while not progress.finished:
                for ind, task in enumerate(tasks): 
                    ...
                    finished_pixels = sub_data_con[np.logical_not(np.isnan(sub_data_con))].size
                    if np.abs((print_counter-datetime.datetime.now()).total_seconds()) > 10:
                        print("finished_pixels =",finished_pixels,"/",sub_data_con.size)
                        print_counter = datetime.datetime.now()
...
class WindowFit():
    ...
    def __init__(   
        ...
        if isinstance(hdu,Iterable):
            assert (
                len(hdu)==len(init_params)==len(quentities)
            )
            if isinstance(window_names,Iterable):
                assert (
                    len(hdu)==len(window_names)
                )
        self.hdu                     = hdu                                          
        self.init_params             = init_params                          
        self.quentities              = quentities                            
    ...
    def run_preparations(self,redo=False):
        ...
        self._Chianti_window_names   = self.get_CHIANTI_lineNames([name['name'] for name in self.window_names]) if self.window_names is not None else None
    ...
    def polyHDU_preparation(self):
        if len(self.separate_windows) == len(self.hdu): 
            if self.verbose>-1 : print('the separate windows have been generated already if so the code may break as the variables of this instance have been already adapted [It\'s going to be reinitiated to old values first]')
            ...
            for ind,hdu in enumerate(self.hdu):
                ...
                self.separate_windows.append(
                    WindowFit(
                        ...
                        fit_func                = self.fit_func[ind] if self.fit_func is Iterable else self.fit_func                     ,
                        window_names            = self.window_names[ind] if isinstance(self.window_names,Iterable) else self.window_names,
                        bounds                  = self.bounds                                                                            ,
                        window_size             = self.window_size                                                                       ,
                        convolution_function    = self.convolution_function                                                              ,
    ...
    def build_fused_FitFunc(self):
        B_ind = []
        for ind,window in enumerate(self.separate_windows):
            B_ind.append([find_nth_occurrence(window.quentities,"B",n+1) for n in range(window.quentities.count("B"))])
        ...
        for ind in range(len(init_params)):
            ...
            ind_originaleInFused.append(
                ...
                [
                    ind_fused,
                    ind_fused + len(init_params_empty[ind]) - (len(B_ind[ind]) if self.share_B else 0)
                    ]
            )
        ...
        selected_func = str_func_differentB_TEMPLATE if not self.share_B else str_func_sharedB_TEMPLATE
        separate_init_str = "\n"
        ...
        str_func = "\n".join([str_import_Template.format(i.__name__) for i in self.import_function_list if i!=flat_inArg_multiGauss ])+str_func
    ...
    def _get_sigma_data(self,redo= False):
        if self.verbose>=0: print("Computing uncertainties")
    ...
    def _convolve(self,redo= False):
        if self.verbose >= 0: print('Convolving data')
    ...
    def _denoise(self,redo= False):
        if self.verbose >= 0: print('Denoising data')
    ...
    def _Gen_output_shared_memory(self):
        if self.verbose >= 0: print('Generating input/output shared memory')
        dshape  = np.array([self.conv_data.shape[1],
        ...
        self._war = {"name":self._shmm_war.name,"dtype":self.conv_data.dtype,"shape":self.conv_data.shape}
    ...
    def write_data(self):
        ...
        if self.data_filename is None:
            if self.verbose >=0: print(r'You haven\'t specified so It\'s going to be set as "solo-spice-L2.5_{specific_param_val}.fits" ' )
            self.data_filename = (
    ...
    def task_fit_pixel(x:np.ndarray                     ,
        ...
        for index in list_indeces:
            ...
            while True: #this will break only when the convolution threshold is met or reached max allowed convolutions
                ...
                if data_war[i_ad,0,:,i_y,i_x][np.logical_not(np.isnan(data_war[i_ad,0,:,i_y,i_x]))].shape[0]>=ini_params.shape[0]:
                    locked_ini_params,locked_quentities,_,_ = gen_locked_params(ini_params,quentities,lock_protocols) 
                    locked_last_par,locked_last_cov = fit_pixel_multi(x =x,
                                                    y=data_war[i_ad,
                                                                0,
                                                                :,
                                                                i_y,
