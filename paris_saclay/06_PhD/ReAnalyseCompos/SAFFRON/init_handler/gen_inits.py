...
class GenInits:
    def __init__(self,
        ...
        if line_catalogue is None:
            ...
            raise ValueError("line_catalogue must be a LineCatalog object or a path to a line catalog file \n else None to use the internal catalogue")
        ...
        self.windows_lines = {}
        self.global_shift = {"SW":0,"LW":0} # this is the global shift that is going to be used to shift the lines to the right position compared to the catalog
        pd.options.mode.chained_assignment = None
        

        # Ignore FITSFixedWarning
    
    def _gen_inits_window(self,hdul_index:int,verbose:int=None,ax=None
    )->Dict[str, Any]:
        """
        Description: Generate initial parameters for fitting spectral data using Gaussian functions.
        """
        if verbose is None: vb = self.verbose; 
        else: vb = verbose
        if vb >= 3 or vb ==-4 :
            if ax is None:
        ...
        if True:
            ...
            specdata = np.array([(specdata[i] 
                                if 
                                ((s.start if s.start>0 else len(specdata)-s.start) if s.start is not None else 0) 
                                <= i < 
                                ((s.stop if s.stop>0 else len(specdata)-s.stop) if s.stop is not None else len(specdata)) 
                                else 
                                np.nan ) for i in range(len(specdata))]) 
            
            if len(specdata[(~ np.isnan(specdata))]) <=2: raise(ValueError("The spectrum is nearly or fully empty wvl_interval is not correct"))
        ...
        if True:
            ...
            for i in range(n_expensions):
                ...
                for ind in range(len(self.default_lines)):
                    ...
                    min_wvl = specaxis[~ np.isnan(specdata)][ 0] - expansion - self.global_shift[WV]
                    max_wvl = specaxis[~ np.isnan(specdata)][-1] + expansion - self.global_shift[WV]
                    if min_wvl <= line["wvl"] <= max_wvl:
                        window_lines.append(line)
                        line_index = np.nanargmin(np.abs(specaxis-line['wvl']))
                        window_lines[-1]["index"] = line_index # adding the index of the line in the spectrum
                if  len(window_lines)==0:
                    if vb >= -1 :
                        print(f"no lines found in the window {kw} with the global shift {self.global_shift[WV]}")
                        if i<n_expensions:
                            print(f"expanding the boundaries by from by {expansion+expension_factor} Angstroms")
                else:
                    break 
        ...
        if True:
            ...
            for line in window_lines:
                init_param_theory.extend([
                    0.05,
                    line['wvl']+self.global_shift[WV], 0.35
                ])
        ...
        if True:
            if vb >= 3 or vb < -2:
                ...
                ax.step(specaxis, flat_inArg_multiGauss(specaxis, *init_param_theory),
            ...
            if vb >= 2:
                ...
                for i, line in enumerate(window_lines):
                    print(f"\t{line['name']:<8} at {line['wvl']:06.1f} in index {line['index']}")
                print(f"_______________________________________")
            # finding the closest line to the max spectral line and adjust init_params_theory accordingly
            init_param_maxAdjusted, specaxis, specdata = self.find_by_default_window(specaxis, specdata, init_param_theory, window_lines, verbose=vb,catalog=self.catalog)
            if vb >= 3 or vb < -2:
                ax.step(specaxis, flat_inArg_multiGauss(specaxis, *init_param_maxAdjusted),
                        label="default windows adjustement{}".format(" NANs" if np.isnan(init_param_maxAdjusted).any()else ""))
        ...
        if True:
            if len(init_param_maxAdjusted) > 4:  # fitting with one position of the fit
                ...
                init_param_locked, lock_quentities = self.gen_lock_params(init_param_maxAdjusted, lock_state, verbose=vb)
                ...
                if vb >= 3 or vb < -2:
                    ax.step(specaxis, lock_func(specaxis, *init_param_locked),
                            label="Locked fit{}".format(" NANs" if np.isnan(init_param_locked).any()else ""))
                if not np.isnan(init_param_locked).any():
                    init_param_locked = self.get_unlock_params(init_param_locked, lock_state)
                else:
                    if vb >= 0:
                        print("haven't found the right params after locking using the max adjusted params")
        ...
        if True:
            init_param_unlocked, var = fit_pixel_multi(specaxis,
                                            specdata,
            ...
            if not np.isnan(init_param_unlocked).any():
                ...
                if vb >= 0:
                    print("haven't found the right params after unlocking using the locked params")
        ...
        if True:
            ...
            if vb >= 3 or vb < -2:
                ...
                ax.step(specaxis, flat_inArg_multiGauss(specaxis, *init_param_unlocked),
                        label="Unlocked fit{}".format(" NANs" if np.isnan(init_param_unlocked).any() else ""))
        ...
        if True:
            ...
            if self.global_shift[WV] == 0 and not np.isnan(np.nanmean(offset)):self.global_shift[WV] = np.nanmean(offset)  
    ...
    def gen_inits(self,verbose:int=None):
        ...
        for i in range(len(unq)):
            self._gen_inits_window(i,verbose=vb if np.abs(vb)<=3 else 3*(vb/np.abs(vb)),ax=None if abs(vb)<3 else axis[i])
        if vb >= 4 or vb <= -4: 
            os.makedirs("./tmp", exist_ok=True)
            dtime = str(datetime.datetime.now())[:19].replace(":", "-")
    ...
    def find_by_default_window(
        ...
        if True:
            if catalog is None : catalog = LineCatalog(verbose=verbose)
            ...
            if lines_names_sorted == sorted(default_windows[i]):
                ...
                if abs(target_wvl-current_wvl) > 1.5:
                    if verbose >= -1:
                        ...
                        print(f'between {current_wvl-1:06.1f} Angstroms and {current_wvl+1:06.1f} Angstroms')
                    delta_neigbours = 1
                    mod_specdata_neigbors = mod_specdata.copy()*np.nan
                    mod_specdata_neigbors[
                        np.argmin(np.abs(specaxis-(current_wvl-delta_neigbours))):np.argmin(np.abs(specaxis-(current_wvl+delta_neigbours)))
                        ] = mod_specdata[
                        np.nanargmin(np.abs(specaxis-(current_wvl-delta_neigbours))):np.nanargmin(np.abs(specaxis-(current_wvl+delta_neigbours)))]
                ...
                if True:
                    maxB = np.nanmean(sorted(specdata[(~ np.isnan(specdata))])[:max( 5,int(0.05*len(specdata)))])
                    init_params2[-1] = maxB
                    
                    for ind in range(0, len(init_params)//3):
                        wavelength = init_params2[ind*3+1]
                        index = np.nanargmin(np.abs(specaxis-wavelength))
                        init_params2[ind*3] = (init_params2[j*3] if np.isnan(specdata[index]) else specdata[index])
