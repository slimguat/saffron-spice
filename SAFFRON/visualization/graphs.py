    ...
    for i in range(len(q)):
        ...
        if q[i] == "x": 
            ...
            if "raster" in kwargs:
                ...
                im = axis[_c,1].pcolormesh(All_ang_lon,All_ang_lat,(p[i,t]-mean_x)/mean_x*3*10**5, vmin=min_x,vmax=max_x ,cmap=cmap)
            else:
        ...
        if q[i] == "s":
            ...
            if "raster" in kwargs:
                im = axis[_c,2].pcolormesh(All_ang_lon,All_ang_lat,p[i,t],cmap=cmap,vmax = max_s, vmin = min_s)
            else:
        ...
        if q[i] == "B":
            _c += 1
            minmax = {"interval" : AsymmetricPercentileInterval(1, 99)} if type(max_B)==type(None) else {"vmin" : min_B,"vmax" : max_B} 
            good_data = True     
            try:
                norm = ImageNormalize(p[i,t],**minmax,stretch=SqrtStretch())
            except:
                ...
                good_data = False
            if "raster" in kwargs:
                im = axis[_c,0].pcolormesh(All_ang_lon,All_ang_lat,p[i,t] ,norm=norm if good_data else None,cmap="magma")
            else:
                im = axis[_c,0].imshow(p[i,t], aspect= "auto",origin="lower",norm=norm if good_data else None,cmap="magma")
            ...
            if type(convlist) == type(None):
                ...
                if "raster" in kwargs:
                    im = axis[_c,1].pcolormesh(All_ang_lon,All_ang_lat,data_conv ,cmap=cmap, norm=norm)
                else:
...
def plot_sumup( raster     : np.ndarray,
    ...
    if True: #Coordinate matrix    
        ...
        All_ang_lon[All_ang_lon>=180*3600] =  All_ang_lon[All_ang_lon>=180*3600] - 360*3600 
        All_ang_lat[All_ang_lat>=180*3600] =  All_ang_lat[All_ang_lat>=180*3600] - 360*3600 
    if True: #Adjusting quite sun
        if qs[1] < 0:
            if ws[1,1] <0: qs[1] = w.shape[3]+2+ws[1,1]+qs[1]
            else: qs[1] = ws[1,1]+1+qs[1]
        if qs[3] == -1:
            if ws[0,1] <0: qs[3] = w.shape[2]+2+ws[0,1]+qs[3]
            else: qs[3] = ws[0,1]+1+qs[3]
    ...
    for i in range(len(q)):
        if q[i] == "I":
            ...
            im = axis[_c,0].pcolormesh(All_ang_lon,All_ang_lat,data, norm=norm,cmap="magma")
            axis[_c,0].set_title( 'Intensity ($W \cdot m^{-2} \cdot sr^{-1}\cdot nm^{-1}$)',fontsize = title_size )
            ...
            print(
                ...
                "ypos_data"    ,ypos_data    ,"\n",
                "ypos_fig"     ,ypos_fig     ,"\n",
                "texw=t_position",axis_position[0]+ypos[0]*(axis_position[2]),"\n",
                "texw=t_position",axis_position[1]+ypos[1]*(axis_position[3]),"\n",
            ...
            bbox={'facecolor':(0.2,1,0.4),'alpha':0.5,'edgecolor':'black','pad':2},
            fontsize=sup_title_size,rotation=90, va='center',transform=axis[_c,0].transAxes)
            
        if q[i] == "x":
            
            data,_ =gen_velocity(p[i,t],
                        quite_sun=qs,
            ...
            im = axis[_c,1].pcolormesh(All_ang_lon,All_ang_lat,data , vmin=min_V,vmax=max_V,cmap=cmap)
        ...
        if q[i] == "s":
            ...
            cmap = plt.get_cmap('hot').copy()
            cmap.set_extremes(under='green', over='violet') if visualize_saturation else 0
            im = axis[_c,2].pcolormesh(All_ang_lon,All_ang_lat,data,cmap=cmap,vmax = max_W, vmin = min_W)
        ...
        if q[i] == "B":
            ...
            im = axis[_c,0].pcolormesh(All_ang_lon,All_ang_lat,p[i,t] ,norm=norm,cmap="magma")
            axis[_c,0].set_title('Background Intensity\n($W \cdot m^{-2} \cdot sr^{-1}\cdot nm^{-1}$)', fontsize=title_size)
            axis[_c,0].set_xlabel('Helioprojective longitude \n (arcsec)'                             , fontsize=label_size)
            axis[_c,0].set_ylabel('Helioprojective latitude \n (arcsec)'                              , fontsize=label_size) 
            axis[_c,0].set_aspect('equal')
            plt.colorbar(im,ax=axis[_c,0],extend=  ("both" if visualize_saturation else None))
            ...
            if type(locklist) == type(None):
                ...
                im = axis[_c,2].pcolormesh(All_ang_lon,All_ang_lat,locklist[t],cmap=cmap,norm=norm,alpha=1)
    ...
    if True:
        ...
        axis[-1,0].plot([QS_up_leftx,QS_up_rightx,QS_down_rightx,QS_down_leftx,QS_up_leftx],
                        ...
                        "o-",
                        alpha = 0.7,
                        color= (0,1,0),lw=ratio/2,
                        label="selected\nquite region")
        axis[-1,0].plot([WS_up_leftx,WS_up_rightx,WS_down_rightx,WS_down_leftx,WS_up_leftx],
                        [WS_up_lefty,WS_up_righty,WS_down_righty,WS_down_lefty,WS_up_lefty],
                        "o-",
                        alpha = 1,
                        color= (1,0,0),lw=ratio/2,
                        label="Analysed region")
        ...
        for x in mean_pos:
            axis[-1,1].axvline(x,ls=':',label="line: {:02d}".format(mean_pos.index(x)))
        axis[-1,1].legend()
        axis[-1,1].set_title("original average spectrum/\naverage line positions/\n segments",fontsize=ratio*4)
        
        axis[-1,2].remove()
        fig.suptitle((suptitle+"\n") if type(suptitle)!=type(None) else ""+  
    ...
    for num_ions in range(N):
        ...
        for i in range(3):
            if q[i] == "I":
                ...
                norm = ImageNormalize(data,**minmax,stretch=SqrtStretch())
                im = axI.pcolormesh(All_ang_lon,All_ang_lat,data, norm=norm,cmap="magma")
                axI.set_title('Intensity ($W \cdot m^{-2} \cdot sr^{-1}\cdot nm^{-1}$)' ,fontsize = title_size)
                axI.set_xlabel('Helioprojective longitude \n (arcsec)'                  ,fontsize = label_size)
                axI.set_ylabel('Helioprojective latitude \n (arcsec)'                   ,fontsize = label_size)
                axI.set_aspect('equal'                                                  )
                ...
                cbar = plt.colorbar(im,shrink=0.75,ax=axI,extend=  ("both" if visualize_saturation else None))
                cbar.ax.tick_params(labelsize=cbar_tick_size)
                
            if q[i] == "x": 
                axx = fig.add_subplot(gs0[1,0])
                data,_ =gen_velocity(p[i+3*num_ions,t],
                        quite_sun=qs,
                        correction=maxwell_correction,
                        verbose=verbose)
                if type(locklist)!=type(None) and num_ions == lockline:
                ...
                #print("Calculated mean: ", mean_x)
                im = axx.pcolormesh(All_ang_lon,All_ang_lat,data, vmin=min_V,vmax=max_V ,cmap=cmap)
                ...
                cbar = plt.colorbar(im,shrink=0.75,ax=axx,extend=  ("both" if visualize_saturation else None))
                cbar.ax.tick_params(labelsize=cbar_tick_size)
            if q[i] == "s":
                ...
                cmap = plt.get_cmap('hot').copy()
                cmap.set_extremes(under='green', over='violet') if visualize_saturation else 0
                im = axs.pcolormesh(All_ang_lon,All_ang_lat,p[i+3*num_ions,t],cmap=cmap,vmax = max_W, vmin = min_W)
                ...
                cbar = plt.colorbar(im,shrink=0.75,ax=axs,extend=  ("both" if visualize_saturation else None))
                cbar.ax.tick_params(labelsize=cbar_tick_size)
        ...
        minmax = {"interval" : AsymmetricPercentileInterval(1, 99), "vmin" : None,"vmax" : None}    
        # if type(Max_I)!=type(None) and type(Min_I)!=type(None):s
        
        norm = ImageNormalize(mn,**minmax,stretch=SqrtStretch())
        im = axAI.pcolormesh(All_ang_lon,All_ang_lat,mn,norm=norm,cmap="magma")
        # axAI.set_aspect('auto')
        cbar = plt.colorbar(im,shrink=0.75,ax=axAI,extend=  ("both" if visualize_saturation else None))
        cbar.ax.tick_params(labelsize=cbar_tick_size)
        ...
        if type(segmentation) != type(None):
            if len(segmentation.shape)!=1:
                ...
                axAS.axvspan(seg[0], seg[1], alpha=0.5,color = color)
        plt.tight_layout()
        if save: plt.savefig(filename.format(num_ions))
