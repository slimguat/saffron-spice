...
class GenInits:
    def __init__(
        ...
        if line_catalogue is None:
            ...
            self.catalog = LineCatalog(
                verbose=verbose, file_location=line_catalogue)
        else:
            raise ValueError(
    ...
    def gen_inits_window(
        ...
        if True:
            ...
            if len(window_lines) == 0:
                ...
                raise ValueError(
                    f"\033[93mWARNING: no lines found in the window {kw}\033[0m")
            ...
            Free_params_Model.add_polynome(max(0.00, np.nanmean(specdata)), name="Bg", lims=[
                                           specaxis[0], specaxis[-1]+(specaxis[-1]-specaxis[-2])])
        ...
        if True:
            if vb >= 3 or vb < -2:
                ax.step(specaxis, np.nanmean(hdu.data, axis=(0, 2, 3)).astype(
                    float), label=f"original spectrum", color='black', ls=":")
                ax.step(specaxis, specdata2, color='black')
                ax.step(
                    specaxis,
                    Free_params_Model.callables['function'](
                        specaxis, *init_param_theory),
        ...
        if True:
            ...
            if not np.any(np.isnan(init_param_maxAdjusted)):
                maxAdjusted_params_Model.set_unlock_params(
                    init_param_maxAdjusted)
            if vb >= 3 or vb < -2:
                ax.step(
                    specaxis,
                    Free_params_Model.callables['function'](
                        specaxis, *init_param_maxAdjusted),
                    label="default windows adjustement{}".format(
                        " NANs" if np.isnan(
                            init_param_maxAdjusted).any() else ""
                    ),
                )
        ...
        if True:
            ...
            if len(init_param_maxAdjusted) > 4:  # fitting with one position of the fit
                ...
                all_locked_params_Model.set_unlock_params(
                    init_param_maxAdjusted)
                for gauss_i in range(1, len(window_lines)):
                    diff = Free_params_Model.functions['gaussian'][0]['x'] - \
                        Free_params_Model.functions['gaussian'][gauss_i]['x']
                    all_locked_params_Model.lock(
                        {'model_type': 'gaussian',
                            'element_index': gauss_i, 'parameter': 'x'},
                        {'model_type': 'gaussian',
                            'element_index': 0, 'parameter': 'x'},
                        {'operation': 'add', 'value': -diff}
                    )
                    all_locked_params_Model.lock(
                        {'model_type': 'gaussian',
                            'element_index': gauss_i, 'parameter': 's'},
                        {'model_type': 'gaussian',
                            'element_index': 0, 'parameter': 's'},
                ...
                if vb >= 3 or vb < -2:
                    ax.step(
                        ...
                        label="Locked fit{}".format(
                            " NANs" if np.isnan(
                                init_param_locked).any() else ""
                        ),
                    )
                if not np.isnan(init_param_locked).any():
                    init_param_locked = all_locked_params_Model.get_unlock_params(
                        init_param_locked)
                else:
                    if vb >= 0:
                        print(
        ...
        if True:
            ...
            if vb >= 3 or vb < -2:
                ...
                for i, l in enumerate(init_param_unlocked[array_quentity == "x"]):
                    ax.axvline(
                        l, label=f"{window_lines[i]['name']},{l:6.1f}", ls=":")
                try:
                    all_unlocked_Model.callables['function'](
                        specaxis, *init_param_unlocked)
                except:
                ...
                if np.abs(vb) >= 4:
                    ...
                    try:
                        ax.get_figure().savefig(
                            f"./tmp/{dtime}_window{hdul_index}.jpg")
                    except:
    ...
    def gen_inits(self, extend_wvl_search=0.5*u.Angstrom, verbose: int = None):
        ...
        if vb >= 3 or vb <= -3:
            ...
            axis = axis.flatten()
        try:
            list(extend_wvl_search)
        except:
            extend_wvl_search = [extend_wvl_search]*len(unq)
    ...
    def plot_overview(self, verbose: int = None):
        ...
        for i, params in enumerate([model.get_unlock_params() for model in self.Models]):
            ...
            ax2[i].step(specaxis, self.Models[i].callables['function'](
                specaxis, *params))
    ...
    """
        ...
        is_strict = np.array([True if min_val <= float(
            wvl) <= max_val else False for wvl in np.array(sub_window_lines)[:, 1]])
        functions = sub_functions.copy()
        functions['gaussian'] = [sub_functions['gaussian'][i]
                                 for i in range(len(sub_functions['gaussian'])) if is_strict[i]]
        functions['gaussian'] = {i: functions['gaussian'][i]
                                 for i in range(len(functions['gaussian']))}
        window_lines = Model.functions_names['gaussian']

        window_lines = [sub_window_lines[i]
                        for i in range(len(sub_window_lines)) if is_strict[i]]
        Model = ModelFactory(jit_activated=False,
                             functions=functions, functions_names=window_lines)
        ...
        for i in range(len(default_windows)):
            if set(lines_IDs) == set(default_windows[i]):
                ...
                if True:
                    ...
                    init_params2[0:l:3] = init_params2[0:l:3] - \
                        init_params2[-1]
                    # print(init_params2[0:l:3])
                init_params2[0:l:3][init_params2[0:l:3]
                                    <= 0] = 0.05 - init_params2[-1]
