...
def fit_pixel_model(
    ...
    **kwargs,
    ...
    """
    ini_params = Model.get_lock_params()
    bounds = Model.bounds
    # print("bounds shape", np.array(bounds).shape)
    fit_func = Model.callables['function']
    fit_jac = Model.callables['jacobian']
    # print(f"fit_func location : {Model.func_path}")
    ...
    if plotit or verbose >= 3 or verbose == -3:
        ...
        quentities = Model.get_lock_quentities()
...
def fit_pixel_multi(
    ...
    **kwargs,
    ...
    if _y.shape[0] <= _s:
        ...
        return (np.ones((_s)) * np.nan, np.ones((_s, _s)) * np.nan)
    try:
        res = curve_fit(fit_func, _x, _y, p0=ini_params, bounds=bounds, sigma=w, jac=jac_func)
    except RuntimeError:
        plotit = True if np.random.random() * 1 >= np.inf else False
        if verbose >= 1:
            print(
