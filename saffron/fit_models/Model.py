...
class ModelFactory():
    def __init__(self, functions=None, functions_names=None, jit_activated=True, cache=True, verbose=0) -> None:
        ...
        """
        self.string_to_function_generator_list = {
            'gaussian': self._gaussian_string_to_function, 'polynome': self._polynome_string_to_function}
        self.string_to_jacobian_generator_list = {
            'gaussian': self._gaussian_string_to_jacobian, 'polynome': self._polynome_string_to_jacobian}
    ...
    def __add__(self, other: 'ModelFactory') -> 'ModelFactory':
        ...
        """
        if type(other).__name__ != ModelFactory.__name__:
            raise ValueError("Can only add Model to Model")
            ...
            for key2 in b_dict[key]:
                ...
                for key3 in b_dict[key][key2]:
                    if not isinstance(b_dict[key][key2][key3], dict):
                        ...
                        func_prams[key3] = copy.deepcopy(
                            b_dict[key][key2][key3])
    ...
    def __repr__(self) -> str:
        ...
        def format_locked_parameter(param_dict):
            ...
            reference_value = self.functions[reference["model_type"]
                                             ][reference["element_index"]][reference["parameter"]]
            calculated_value = (
            ...
            for key2, param_dict in key_dict.items():
                ...
                for param_name, param_value in param_dict.items():
                    if isinstance(param_value, dict) and param_value.get("constraint") == "lock":
                        formatted_params[param_name] = format_locked_parameter(
                            param_value)
                    else:
                ...
                output += "\n   " + \
                    "\n   ".join([f"{param}={value}" for param,
                                 value in formatted_params.items()])
    ...
    def add_polynome(self, *Bi: float, lims: list = [-np.inf, np.inf], name=None) -> None:
        ...
        self.functions['polynome'][index] = {
            **{f'B{i}': Bi[i] for i in range(len(Bi))}, 'lims': lims}
    ...
    def lock(self, param_1: dict, param_2: dict, lock_protocol: dict) -> None:
        ...
        """
        assert "model_type" in param_1.keys(
        ) and "element_index" in param_1.keys() and "parameter" in param_1.keys()
        assert "model_type" in param_2.keys(
        ) and "element_index" in param_2.keys() and "parameter" in param_2.keys()
        ...
        if param_1['model_type'] == 'polynome' or param_2['model_type'] == 'polynome':
            raise ValueError(
                "Locking into polynoms is forbidden because of mask conflict reasons")
        ...
        if model2[p2] == 'lock':
            raise ValueError(f"Parameter {p2} is locked")
            ...
            for element_index in range(len(self.functions[model_type])):
                for parameter in self.functions[model_type][element_index]:
                    if (isinstance(self.functions[model_type][element_index][parameter], dict) or parameter == "lims"):
                        continue
                    self.lockParameterIndexMapping.append(
                        [model_type, element_index, parameter])
        self.lockParameterIndexMapping = np.array(
            self.lockParameterIndexMapping, dtype=object)
        ...
        for model_type in self.functions:
            for element_index in range(len(self.functions[model_type])):
                for parameter in self.functions[model_type][element_index]:
                    if (parameter == "lims"):
                        continue
                    self.unlockParameterIndexMapping.append(
                        [model_type, element_index, parameter])
        self.unlockParameterIndexMapping = np.array(
            self.unlockParameterIndexMapping, dtype=object)
    ...
    def get_lock_params(self, unlocked_params=None) -> np.ndarray:
        ...
        if unlocked_params is None:
            ...
            if len(unlocked_params) != len(self.unlockParameterIndexMapping):
                raise ValueError(
                    "Unlocked parameters must be equal to the number of unlocked parameters")

            locked_params = np.zeros(len(self.lockParameterIndexMapping))
            for index_lock, lock in enumerate(self.lockParameterIndexMapping):

                index_unlock = self.get_locked_index(
                    self.unlockParameterIndexMapping, lock)
        ...
        if locked_params is None:
            ...
            for index_unlock, lock in enumerate(self.unlockParameterIndexMapping):
                if not isinstance(self.functions[lock[0]][lock[1]][lock[2]], dict):
                    unlocked_params[index_unlock] = self.functions[lock[0]
                                                                   ][lock[1]][lock[2]]
                else:
                    constraint = self.functions[lock[0]
                                                ][lock[1]][lock[2]]['constraint']
                    if constraint == 'lock':
                        reference = self.functions[lock[0]
                                                   ][lock[1]][lock[2]]['reference']
                        operation = self.functions[lock[0]
                                                   ][lock[1]][lock[2]]['operation']
                        value = self.functions[lock[0]
                                               ][lock[1]][lock[2]]['value']
                        if operation == 'add':
                            unlocked_params[index_unlock] = value + self.functions[reference['model_type']
                                                                                   ][reference['element_index']][reference['parameter']]
                        else:
                            unlocked_params[index_unlock] = value * self.functions[reference['model_type']
                                                                                   ][reference['element_index']][reference['parameter']]
                        ...
                        raise ValueError(
                            f"Constraint {constraint} not supported")

        else:
            unlocked_params = np.zeros(len(self.unlockParameterIndexMapping))
            for index_unlock, lock in enumerate(self.unlockParameterIndexMapping):
                if not isinstance(self.functions[lock[0]][lock[1]][lock[2]], dict):
                    index_lock = self.get_locked_index(
                        self.lockParameterIndexMapping, lock)
                    ...
                    if constraint == 'lock':
                        ...
                        index_lock_reference = self.get_locked_index(
                            self.lockParameterIndexMapping, reference.values())
                        if operation == 'add':
                            unlocked_params[index_unlock] = value + \
                                locked_params[index_lock_reference]
                        else:
                            unlocked_params[index_unlock] = value * \
                                locked_params[index_lock_reference]
                    else:
            ...
            for model_type in self.functions:
                for ind_func in (self.functions[model_type]):
                    ...
                    self.function_string[1] += "\n\t" + \
                        self.string_to_function_generator_list[model_type](
                            ind_func)
            self.function_string[1] += '\n\treturn sum'

        if True:  # Generate the jacobian string
            # _{}_{}'.format(now_date.strftime("%Y%m%d"),now_date.strftime("%H%M%S"))
            _name2 = 'model_jacobian'
            ...
            lines = [
                "jac[{}"+f",{i}]=" for i in range(len(self.lockParameterIndexMapping))]
            mask_declarations = []
            for model_type in self.functions:
                for ind_func in (self.functions[model_type]):
                    params_jac = self.string_to_jacobian_generator_list[model_type](
                        ind_func)
                    """
          params_jac = [
    ...
    def dump_function(self, directory: Path = Path('./tmp')) -> None:
        ...
        while True:
            ...
            self.func_path = directory / \
                f'Model_functions_{now_date.strftime("%Y%m%d")}_{now_date.strftime("%H%M%S")}_{randomint}.py'
    ...
    def _lock_arg_string_generator(self, model_type: str, ind_func: int, key: str) -> str:
        ...
        if self.functions[model_type][ind_func][key]['operation'] == 'add':
            ...
            raise ValueError(
                f"Operation {self.functions[model_type][ind_func][key]['operation']} not implemented")
    ...
    def _gaussian_string_to_function(self, ind_func: int) -> str:
        ...
        for key, item in self.functions["gaussian"][ind_func].items():
            if not isinstance(item, dict):
                str_params.append(
                    f'arg_{self.get_locked_index(self.lockParameterIndexMapping,["gaussian",ind_func,key])}')

            elif self.functions["gaussian"][ind_func][key]["constraint"] == 'lock':
                str_params.append(self._lock_arg_string_generator(
                    "gaussian", ind_func, key))
            else:
                raise ValueError(
                    f"Constraint {self.functions['gaussian'][ind_func][key]['constraint']} not implemented")
    ...
    def _polynome_string_to_function(self, ind_func: int) -> str:
        ...
        if True:
            ...
            if not (lims[0] == -np.inf and lims[1] == np.inf):
                ...
                mask_array = "mask_array_{2} = (x > {0}) & (x < {1})".format(
                    *lims, rand)
            ...
            for key, item in sorted(self.functions["polynome"][ind_func].items(), reverse=True):
                if key != 'lims':
                    if not isinstance(item, dict):
                        str_params.append(
                            f'arg_{self.get_locked_index(self.lockParameterIndexMapping, ["polynome", ind_func, key])}')
                    elif self.functions["polynome"][ind_func][key]["constraint"] == 'lock':
                        str_params.append(self._lock_arg_string_generator(
                            "polynome", ind_func, key))
                    else:
                        raise ValueError(
                            f"Constraint {self.functions['polynome'][ind_func][key]['constraint']} not implemented")

            # Generate the Horner's method polynomial evaluation string
            # Start with the coefficient of the highest power
            horner_expression = str_params[0]
    ...
    def _gaussian_string_to_jacobian(self, ind_func: int) -> list:
        ...
        if not isinstance(I, dict):
            Ival = f'arg_{self.get_locked_index(self.lockParameterIndexMapping,["gaussian",ind_func,"I"])}'
        else:
            Ival = self._lock_arg_string_generator("gaussian", ind_func, "I")
        if not isinstance(x, dict):
            xval = f'arg_{self.get_locked_index(self.lockParameterIndexMapping,["gaussian",ind_func,"x"])}'
        else:
            xval = self._lock_arg_string_generator("gaussian", ind_func, "x")
        if not isinstance(s, dict):
            sval = f'arg_{self.get_locked_index(self.lockParameterIndexMapping,["gaussian",ind_func,"s"])}'
        else:
            sval = self._lock_arg_string_generator("gaussian", ind_func, "s")

        # add derivation of lock values, and its typical index in the jacobian matrix
        if not isinstance(I, dict):
            dIval = ""
            dIval_ind = self.get_locked_index(self.lockParameterIndexMapping, [
                                              "gaussian", ind_func, "I"])
        else:
        ...
        if not isinstance(x, dict):
            dxval = ""
            dxval_ind = self.get_locked_index(self.lockParameterIndexMapping, [
                                              "gaussian", ind_func, "x"])
        else:
        ...
        if not isinstance(s, dict):
            dsval = ""
            dsval_ind = self.get_locked_index(self.lockParameterIndexMapping, [
                                              "gaussian", ind_func, "s"])
        else:
        ...
        I_string_expression = I_jacobian_template.format(
            Ival, xval, sval, dIval, dxval, dsval)
        x_string_expression = x_jacobian_template.format(
            Ival, xval, sval, dIval, dxval, dsval)
        s_string_expression = s_jacobian_template.format(
            Ival, xval, sval, dIval, dxval, dsval)
        ...
        list_expressions_and_indices = [
            ...
            [exponent_template.format(
                Ival, xval, sval, dIval, dxval, dsval), -1],
        ]
            ...
            for key in self.functions["polynome"][ind_func]:
                if key != 'lims':
                    ...
                    if not isinstance(B, dict):
                        ...
                        Bval = self._lock_arg_string_generator(
                            "polynome", ind_func, key)

                    # Determine the index for the Jacobian matrix
                    if not isinstance(B, dict):
                        dBval = ""
                        dBval_ind = self.get_locked_index(self.lockParameterIndexMapping, [
                                                          "polynome", ind_func, key])
                    else:
                        _ = self._lock_arg_string_jac_generator(
                            "polynome", ind_func, key)
                        dBval = _[0] + ("*" if _[0] != "" else "")
                        dBval_ind = _[1]

                    # Optimize the Jacobian expression using repeated multiplication for exponents
                    # Extract the exponent from the key (e.g., 'B1' -> 1)
                    exponent = int(key[1:])
                    if exponent == 0:
                    ...
                    list_expressions_and_indices.append(
                        [B_string_expression, dBval_ind])

                    if mask_array is not None:
                        list_expressions_and_indices[-1].append(
                            mask_array_name)

            if mask_array is not None:
                list_expressions_and_indices.append(
                    [f"{mask_array};{sub_x}", -1])
    ...
    def reset_callables(self, regenerate_file=True) -> Dict[str, Callable]:
        ...
        spec = importlib.util.spec_from_file_location(
            self.function_string[0], func_file_path)
        ...
        self._callables = {'function': model_function,
                           'jacobian': model_jacobian}
        return self._callables

    def set_bounds(
        self,
        kwargs: Dict[str, Union[List[float],
            ...
            if parameter in kwargs:
                if isinstance(kwargs[parameter][0], Iterable):
                    if kwargs[parameter][0][0] == 'ref-add':
                        self.bounds[0][ind] = self.functions[model_type][element_index][parameter] + \
                            kwargs[parameter][0][1]
                    else:
                        raise ValueError(
                            f"Constraint {kwargs[parameter][0][0]} not supported")
                else:
                    self.bounds[0][ind] = kwargs[parameter][0]
                if isinstance(kwargs[parameter][1], Iterable):
                    if kwargs[parameter][1][0] == 'ref-add':
                        self.bounds[1][ind] = self.functions[model_type][element_index][parameter] + \
                            kwargs[parameter][1][1]
                    else:
                        raise ValueError(
                            f"Constraint {kwargs[parameter][1][0]} not supported")
                else:
    ...
    def set_unlock_params(self, unlocked_params: np.ndarray) -> None:
        ...
        for ind, unlock_val in enumerate(self.unlockParameterIndexMapping):
            ...
            if isinstance(val, dict):
                if self.verbose >= -1:
                    print(
                        f"the value of {unlock_val} is locked not going to be set")
            else:
                self.functions[unlock_val[0]][unlock_val[1]
                                              ][unlock_val[2]] = unlocked_params[ind]
        self.set_bounds()
    ...
    def set_lock_params(self, locked_params: np.ndarray) -> None:
        ...
        for ind, lock_val in enumerate(self.lockParameterIndexMapping):
            self.functions[lock_val[0]][lock_val[1]
                                        ][lock_val[2]] = locked_params[ind]
        self.set_bounds()
    ...
    def plot_model(self, ax: Optional[List[Any]] = None, label: str = "") -> List[Any]:
        ...
        lims = [
            np.min(unlock_params[unlock_quentities == "x"]) -
            np.max(unlock_params[unlock_quentities == "s"])*4,
            np.max(unlock_params[unlock_quentities == "x"]) +
            np.max(unlock_params[unlock_quentities == "s"])*4,
        ]
        ...
        if ax is None:
            fig, ax = plt.subplots(1, 2, figsize=(
                10, 5), gridspec_kw={'wspace': 0})
        ax[0].plot(x, y, label=label)
        ax[0].set_title("model")
        for i in range(len(lock_params)):
            ax[1].plot(x, dy[:, i]/np.max(dy[:, i]),
                       label=f'{self.lockParameterIndexMapping[i][0]},{self.lockParameterIndexMapping[i][1]},{self.lockParameterIndexMapping[i][2]}')
        ax[1].legend()
    ...
    def get_unlock_covariance(self, locked_cov: np.ndarray) -> np.ndarray:
        ...
        for index_unlock, lock in enumerate(self.unlockParameterIndexMapping):
            if not isinstance(self.functions[lock[0]][lock[1]][lock[2]], dict):
                index_lock = self.get_locked_index(
                    self.lockParameterIndexMapping, lock)
                unlock_covariance[index_unlock,
                                  index_unlock] = locked_cov[index_lock, index_lock]
            else:
                constraint = self.functions[lock[0]
                                            ][lock[1]][lock[2]]['constraint']
                if constraint == 'lock':
                    ...
                    if operation == 'add':
                        unlock_covariance[index_unlock,
                                          index_unlock] = locked_cov[index_lock_reference, index_lock_reference]
                    else:
                        unlock_covariance[index_unlock, index_unlock] = value**2 * \
                            locked_cov[index_lock_reference,
                                       index_lock_reference]
                else:
    ...
    def to_hdu(self, hdu_name="FIT_MODEL") -> fits.BinTableHDU:
        ...
        hdu = fits.BinTableHDU.from_columns([
            fits.Column(name='model', format=f'{len(compressed_array)}B', array=[
                        compressed_array])
        ])
        hdu.header['EXTNAME'] = hdu_name
        hdu.header['MODELCLS'] = str(
            self.__class__.__name__)  # Store class name
    ...
    def from_hdu(cls, hdu: fits.BinTableHDU) -> "ModelFactory":
        ...
        if hdu.header.get('COMPTYPE') != 'zlib':
            raise ValueError(
                "Unsupported compression type or missing compression metadata.")

        # Extract and decompress the binary data
        compressed_model = np.array(
            hdu.data['model'][0], dtype=np.uint8).tobytes()
    ...
    def visualize(
        ...
        """
        assert isinstance(
            latex_enable, bool), "`latex_enable` must be a boolean."
        assert isinstance(
            show_values, bool), "`show_values` must be a boolean."
        assert isinstance(
